document.addEventListener('DOMContentLoaded', () => {
    ...
    if (lastOrder && orderDetailsContainer) {
        // Generate and display confirmation number
        const confirmationNumber = Date.now().toString().slice(-8);
        const confirmationText = {
            de: `Ihre Bestätigungsnummer lautet:`,
            en: `Your confirmation number is:`
        };
        document.getElementById('confirmationNumber').innerHTML = `<strong>${confirmationText[lang]}</strong> #${confirmationNumber}`;
